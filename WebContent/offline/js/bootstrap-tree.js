!function ($) {
    ...
    Node = function (element, options) {
        ...
        if (this.type === 'checkbox') {
            ...
            var inputSpan = $('<span class="tree-node tree-checkbox" style=""></span>'),
                input = $('<input  class="node-input" type="checkbox"></input>'),
                custom = this.element.data('input-custom') || options.input.custom,
                customSpan = $('<span class="node-custom"></span>');
            if (this.value) {
    ...
    Node.prototype = {
        ...
        isSubrootVisible: function () {
            var visible = false;
            if (this.subroot !== null  && this.subroot !== undefined) {
        ...
        toggle: function () {
            if (this.subroot !== null && this.subroot !== undefined) {
                this.subroot.each(function () {
                    $(this).data('root').toggle(this.animation);
                });
        ...
        propagate: function (state) {
            if (state === undefined) {
                ...
                var checkedList, parent = this.element.parent().parent().data('node'),
                    inputs = this.element.find('input[value]');

                if (parent) {
                    parent.propagate();
                }

                checkedList = $.map(inputs, function (val) {
                    if ($(val).prop('checked') === true) {
                        return true;
                    }
                });

                if (checkedList.length === inputs.length) {
                    this.update($.fn.node.State.CHECKED);
                } else {
        ...
        search: function (str) {
            ...
            if (this.subroot) {
                this.subroot.children('li').each(function () {
                    if ($(this).data('node').search(str) === true) {
                        searchResult = true;
                    }
                });
            }
    ...
    $.fn.node = function (option, val) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                data = new Node(this, $.extend({}, $.fn.node.defaults, options));
                $this.data('node', data);
            }
    ...
    $.fn.root = function (option, val) {
        ...
        return this.each(function () {
            var $this = $(this),
                ...
                options = typeof option === 'object' && option;
            if (!data) {
                data = new Root(this, $.extend({}, $.fn.root.defaults, options));
                $this.data('root', data);
            }
            if (typeof option === 'string') {
                data[option](val);
    ...
    $.fn.tree = function (option, val) {
        ...
        return $.each(this, function () {
            ...
            var $this = $(this),
                data = $this.data('tree'),
                options = typeof option === 'object' && $.extend(true, {}, option);
            ...
            if (!data) {
                ...
                if ($this.is('div') || $this.is('input')) {
                    ...
                    //     $.getJSON($this.data('tree-values-url')).done(function (data) {
                    //         console.log('should read json');
                    //     });
                    // }
                ...
                data = this.ul.root($.extend({}, $.fn.root.defaults, options));
                $this.data('tree', data);
    ...
    $.fn.tree.generate = function (data, options) {
        ...
        $(data.child).each(function () {
            ...
            $li.attr('input-value', this.value);
